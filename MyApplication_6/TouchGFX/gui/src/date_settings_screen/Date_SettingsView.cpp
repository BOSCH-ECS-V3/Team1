...
{
	// Update values in current labels on the screen
...
{
	/*
	 * If btn days val up is pressed ,change days label text
	 */
	updated_Days++;
	if(updated_Month ==  January || updated_Month ==  March ||
			updated_Month ==  May || updated_Month ==  July ||
			updated_Month ==  August || updated_Month ==  October ||
			updated_Month ==  December)
	{
		if(updated_Days>31)
		{
			updated_Days = 0;
	...
	if(updated_Month ==  April || updated_Month == June ||
			updated_Month==  September || updated_Month ==  November)
	{
		if(updated_Days>30)
		{
			updated_Days = 0;
...
{
	/*
	 * If btn days val down is pressed ,change days label text
	 */
	updated_Days--;
	if(updated_Days<0)
	{
...
{
	/*
	 * If btn month val up is pressed ,change month label text
	 */
	updated_Month++;
...
{
	/*
	 * If btn month val down is pressed ,change month label text
	 */
	if(updated_Month<0)
	{
		updated_Month=0;
...
{
	/*
	 * If btn year val up is pressed ,change year label text
	 */
	updated_Year++;
...
{
	/*
	 * If btn year val down is pressed ,change year label text
	 */
	updated_Year--;
	Unicode::snprintf(Year_ValBuffer, YEAR_VAL_SIZE, "%d", updated_Year);
	Year_Val.invalidate();
...
{
	// when submit button is clicked , update current date and send it to model
