...
void DefaultViewView::SEND_Error_ID(int Error_ID, int SNOOZE_FLAG_TEMPIN,
		int SNOOZE_FLAG_GAS, int SNOOZE_FLAG_TEMPOUT, int SNOOZE_FLAG_HUMIDITY,
		int SNOOZE_FLAG_PRESSURE, int SNOOZE_FLAG_AMBIENT) {
	...
	if (current_ERROR_ID != 0) // If there is error , update UI
		...
		Unicode::snprintf(Error_codeBuffer, ERROR_CODE_SIZE, "%d",
				current_ERROR_ID);
		Alert_cover.setVisible(true);
		switch (current_ERROR_ID) {
		//------------------------TempIN-----------------------------------
		case 101:
			Unicode::snprintf(Error_textBuffer, ERROR_TEXT_SIZE,
...
void DefaultViewView::current_Sensor_values(int current_Sensor_values[],
	...
	Unicode::snprintf(TempOut_ValBuffer, TEMPOUT_VAL_SIZE, "%d",
			Temperature_Sensor_INSIDE_values);

	Unicode::snprintf(TempIn_ValBuffer, TEMPIN_VAL_SIZE, "%d",
			Temperature_Sensor_OUTSIDE_value);

	Unicode::snprintf(Humidity_ValBuffer, HUMIDITY_VAL_SIZE, "%d",
			Humidity_Sensor_value);

	Unicode::snprintf(Presure_ValBuffer, PRESURE_VAL_SIZE, "%d",
			Pressure_Sensor_value);

	Unicode::snprintf(Ambient_ValBuffer, AMBIENT_VAL_SIZE, "%d",
			Ambient_Light_Sensor_value);

	Unicode::snprintf(Carbon_ValBuffer, CARBON_VAL_SIZE, "%d",
			Gas_Sensor_value);
...
void DefaultViewView::Snooze_btn_pressed() {
	...
	if (current_ERROR_ID == 101 || current_ERROR_ID == 102
			|| current_ERROR_ID == 103) {
		flag_TempIN = 1;
	} else if (current_ERROR_ID == 601 || current_ERROR_ID == 602
			|| current_ERROR_ID == 603) {
		flag_Gas = 1;
	} else if (current_ERROR_ID == 201 || current_ERROR_ID == 202
			|| current_ERROR_ID == 203) {
		flag_TempOUT = 1;
	} else if (current_ERROR_ID == 301 || current_ERROR_ID == 302
			|| current_ERROR_ID == 303) {
		flag_Humidity = 1;
	} else if (current_ERROR_ID == 401 || current_ERROR_ID == 402
			|| current_ERROR_ID == 403) {
		flag_Pressure = 1;
	} else if (current_ERROR_ID == 501 || current_ERROR_ID == 502
			|| current_ERROR_ID == 503) {
		flag_ambient = 1;
	}

	presenter->Snooze_system(snooze_state, flag_TempIN, flag_Gas, flag_TempOUT,
			flag_Humidity, flag_Pressure, flag_ambient);
	/*
	 snooze_state = 0 ;
	 flag_TempIN = 0 ;
	 flag_TempOUT = 0 ;
